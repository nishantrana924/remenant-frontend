    ...
    <div class="flex items-center justify-between w-full pr-4">
        <div class="flex items-center gap-8">
            ...
            <div class="flex flex-col justify-center">
                <h2 class="font-black text-xl text-slate-800 leading-none tracking-tight">Edit Product</h2>
                <div class="flex items-center gap-2 mt-1.5">
                    <span class="h-1 w-1 bg-indigo-500 rounded-full animate-pulse"></span>
                    <p class="text-[9px] text-slate-400 uppercase tracking-[0.2em] font-black">{{ $item->title }}</p>
                </div>
            </div>
        </div>
...
@section('content')
    <form action="{{ route('admin.products.update', $item->id) }}" method="POST" enctype="multipart/form-data" class="space-y-8">
        @csrf
        @method('PUT')
                    ...
                    <div class="p-6 space-y-4">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="text" name="slug" required value="{{ old('slug', $item->slug) }}" 
                                       class="w-full rounded-xl border-gray-200 focus:ring-orange-500 focus:border-orange-500">
                            </div>
                            <div>
                        ...
                        <div>
                            ...
                            <textarea name="description" rows="3" 
                                      class="w-full rounded-xl border-gray-200 focus:ring-orange-500 focus:border-orange-500">{{ old('description', $item->description) }}</textarea>
                        </div>
                        ...
                        <div>
                            ...
                            <textarea name="long_description" id="editor" rows="10" 
                                      class="w-full rounded-xl border-gray-200 focus:ring-orange-500 focus:border-orange-500">{{ old('long_description', $item->long_description) }}</textarea>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <textarea name="benefits" rows="4" 
                                      class="w-full font-mono text-xs rounded-xl border-gray-200 focus:ring-orange-500 focus:border-orange-500">@json($item->benefits)</textarea>
                        </div>
                        <div>
                            <label class="block text-sm font-bold text-gray-700 mb-1">Technical Specs</label>
                            <textarea name="specs" rows="4" 
                                      class="w-full font-mono text-xs rounded-xl border-gray-200 focus:ring-orange-500 focus:border-orange-500">@json($item->specs)</textarea>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="space-y-6">
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                    ...
                    <div class="p-6 space-y-4">
                        ...
                        <div class="flex items-center justify-between">
                            ...
                            <label class="relative inline-flex items-center cursor-pointer">
                                <input type="checkbox" name="is_featured" value="1" {{ $item->is_featured ? 'checked' : '' }} class="sr-only peer">
                    ...
                    <div class="p-6 space-y-4">
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="number" name="mrp" step="0.01" required value="{{ old('mrp', $item->mrp) }}" 
                                       class="w-full rounded-xl border-gray-200 focus:ring-orange-500 focus:border-orange-500">
                            </div>
                            <div>
                                <label class="block text-sm font-bold text-gray-700 mb-1">Selling Price (₹)</label>
                                <input type="number" name="price" step="0.01" required value="{{ old('price', $item->price) }}" 
                                       class="w-full rounded-xl border-gray-200 focus:ring-orange-500 focus:border-orange-500">
                            </div>
                        </div>
                    </div>
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                    ...
                    <div class="p-6 space-y-4">
                        @if($item->image)
                            <div class="mb-4">
                                <p class="text-xs text-gray-500 mb-2 font-bold">Current Image:</p>
                                <img src="{{ asset('storage/' . $item->image) }}" class="h-20 w-20 rounded-lg object-cover border border-gray-200">
                            </div>
                        @endif
                        <div>
                            <label class="block text-sm font-bold text-gray-700 mb-1">Replace Image</label>
                            <input type="file" name="image" class="block w-full text-xs text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-xs file:font-semibold file:bg-orange-50 file:text-[#ea5f06] hover:file:bg-orange-100">
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="sticky bottom-6 bg-white/80 backdrop-blur-md p-4 rounded-2xl shadow-2xl border border-gray-200 flex justify-end gap-3 z-10">
            <button type="button" class="px-6 py-2 rounded-xl text-gray-600 font-bold hover:bg-gray-100 transition">Cancel</button>
            <button type="submit" class="px-8 py-2 bg-[#ea5f06] text-white font-bold rounded-xl shadow-lg hover:bg-[#cf5305] transition transform active:scale-95">Update Product</button>
        </div>
    </form>
