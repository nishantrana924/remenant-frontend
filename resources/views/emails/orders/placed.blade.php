...
@section('content')
    ...
    <p class="p">Your order <strong>#{{ $order->order_number }}</strong> has been successfully placed and is now being processed by our warehouse team. We'll notify you as soon as it ships.</p>
    ...
    <div class="order-summary">
        ...
        <div style="padding: 15px 0; border-bottom: 1px solid #e2e8f0;">
            <div style="font-size: 13px; font-weight: 900; color: #0f172a; text-transform: uppercase;">{{ $item->product->title ?? 'Product' }}</div>
            ...
            @if($item->product && in_array($item->product->product_type, ['combo', 'both']) && $item->product->comboItems->count() > 0)
                <div style="margin-top: 8px; padding-left: 10px; border-left: 2px solid #ff6b00;">
        ...
        <div class="totals" style="margin-top: 20px;">
            ...
            <div class="total-row grand-total" style="padding-top: 15px; border-top: 2px solid #e2e8f0; margin-top: 15px;">
                <span class="total-label" style="color: #0f172a; font-size: 14px;">Total Paid</span>
                <span class="total-value" style="color: #ff6b00; font-size: 20px;">₹{{ number_format($order->total_amount) }}</span>
            </div>
        </div>
    </div>

    <div style="text-align: center; margin-top: 40px;">
        <p class="p" style="font-size: 14px;">Want to see where your package is?</p>
        <a href="{{ route('order.track', ['order_number' => $order->order_number]) }}" class="btn">Track Shipment</a>
    </div>

    <div style="margin-top: 50px; padding-top: 30px; border-top: 1px solid #f1f5f9;">
        <h3 style="font-size: 10px; font-weight: 900; color: #0f172a; text-transform: uppercase; margin-bottom: 10px;">Shipping To</h3>
