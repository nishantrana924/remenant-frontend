...
@section('title', in_array($order->refund_status, ['pending', 'initiated', 'completed']) ? 'Refund Initiated - Remenant' : 'Order Confirmed - Remenant')

@section('content')
<div class="min-h-screen bg-[#FDFCFB] pt-32 pb-20 px-4">
    <div class="max-w-3xl mx-auto">
        @if(in_array($order->refund_status, ['pending', 'initiated', 'completed']))
            <!-- Refund Animation & Header -->
            <div class="text-center mb-12">
                <div class="inline-flex h-20 w-20 items-center justify-center rounded-full bg-rose-100 text-rose-600 mb-8 animate-pulse">
                    <i data-lucide="refresh-cw" class="h-10 w-10"></i>
                </div>
                <h1 class="text-4xl font-black italic tracking-tighter uppercase text-rose-950 mb-2">Refund Initiated</h1>
                <p class="text-slate-400 font-bold uppercase tracking-[0.3em] text-xs">Order ID: {{ $order->order_number }}</p>
                <p class="text-sm text-slate-500 font-medium max-w-md mx-auto mt-4">
                    Your payment was received, but the order could not be fulfilled (it may have been cancelled or went out of stock). We have automatically initiated a full refund to your original payment method.
                </p>
            </div>

            <!-- Refund Summary Card -->
            <div class="bg-white rounded-[3rem] shadow-2xl shadow-rose-100 ring-1 ring-black/[0.03] overflow-hidden">
                <div class="p-8 sm:p-12">
                    <div class="flex flex-col sm:flex-row justify-between items-start gap-8 border-b border-slate-100 pb-10 mb-10">
                        <div>
                            <h3 class="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-4">Customer Details</h3>
                            <p class="text-lg font-black text-slate-900 uppercase tracking-tight">{{ $order->customer_name }}</p>
                            <p class="text-sm text-slate-500 font-bold mt-1">{{ $order->email }}</p>
                        </div>
                        <div class="sm:text-right">
                            <h3 class="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-4">Refund Status</h3>
                            <span class="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-rose-50 text-rose-600 text-[10px] font-black uppercase tracking-widest">
                                <i data-lucide="shield-alert" class="h-3 w-3"></i>
                                {{ str_replace('_', ' ', $order->refund_status) }}
                            </span>
                        </div>
                    </div>

                    <div class="space-y-6">
                        <h3 class="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-6">Cancelled Items</h3>
                    ...
                    <div class="mt-12 pt-8 border-t border-slate-100 flex flex-col sm:flex-row justify-between items-center gap-6">
                        <div>
                            <span class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Total Refund Amount</span>
                            <p class="text-3xl font-black text-rose-600 tracking-tighter mt-1">₹{{ number_format($order->total_amount) }}</p>
                        </div>
                        <div class="flex flex-wrap gap-4">
                            <a href="{{ route('home') }}" class="inline-flex items-center justify-center px-8 py-4 rounded-2xl bg-slate-900 text-white font-black uppercase tracking-widest text-[10px] hover:brightness-110 active:scale-95 transition-all">
                                Return to Store
                            </a>
                        </div>
                    </div>
                ...
                <div class="bg-rose-50/50 p-6 text-center border-t border-slate-100">
                    <p class="text-[9px] font-bold text-rose-700 uppercase tracking-[0.2em]">A refund confirmation email has been sent to {{ $order->email }}</p>
                </div>
            </div>
        @else
